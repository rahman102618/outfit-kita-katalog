...
def build_bulk_product(products, pid, raw_line, link):
    ...
    inline_name = parse_inline_name_from_line(raw_line, pid, link)
    if inline_name and ((not is_probably_product_name(name)) or is_placeholder_product_name(name, pid)):
        name = inline_name

    if (not is_probably_product_name(name)) or is_placeholder_product_name(name, pid):
        warn(
            f"Nama produk no {pid} belum terbaca otomatis.\n"
            "Biasanya ini terjadi karena link pendek/affiliate marketplace memblokir data nama dan gambar.\n"
            "Isi nama manual agar kategori tidak salah."
        )
        manual_name = ask("Nama produk manual", default="").strip()
        if manual_name:
            name = clean_title(manual_name, max_len=160)
        else:
            name = f"Produk {pid}"
