...
def show_menu():
    ...
    if RICH:
        ...
        console.print(Panel(grid, title="[bold green]MENU UTAMA[/]", border_style="green", box=box.ROUNDED, padding=(1, 2)))
    else:
        for no, label in items:
            print(f"{no}. {label}")
...
def products_table(products):
    if RICH:
        table = Table(title="Daftar Produk", box=box.ROUNDED, border_style="bright_black")
        table.add_column("No", justify="center", style="cyan", width=5)
        table.add_column("Nama Produk", style="white")
        table.add_column("Kategori", style="yellow", width=16)
        table.add_column("Harga", style="green", width=13)
        table.add_column("Platform", style="magenta", width=10)

        for p in products:
            p = normalize_product(p)
            table.add_row(p["id"], short(p["name"], 35), p["category"], p["price"], p["platform"])
        console.print(table)
    else:
        for p in products:
            p = normalize_product(p)
            print(f"{p['id']}. {p['name']} | {p['category']} | {p['price']} | {p['platform']}")


def product_summary(p, title="Ringkasan Produk"):
    p = normalize_product(p)
    if RICH:
...
def show_product_before_category(pid, name, price, platform, link):
    ...
    if RICH:
        ...
        print("\nProduk sebelum pilih kategori")
        print(f"No Produk : {pid}")
        print(f"Nama      : {name}")
        print(f"Harga     : {price or 'Cek harga'}")
        print(f"Platform  : {platform or detect_platform(link)}")
        print(f"Link      : {link}")
