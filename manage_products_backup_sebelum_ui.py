...
def ask(label, default=None):
    ...
    prompt = term_color(clean_label, "cyan", bold=True) + term_color(default_text + ": ", "yellow", bold=True)
...
def ask_yes(label, default=True):
    ...
    value = input(term_color(clean_label, "yellow", bold=True) + term_color(f" ({pilihan}): ", "cyan", bold=True)).strip().lower()
    if not value:
        return default
    return value == "y"
...
def show_header():
    """Header berwarna untuk Termux, tetap bagus walau rich belum terpasang."""
    meta = datetime.now().strftime("%Y-%m-%d %H:%M")
    git = git_status_text()
    if RICH:
        title = Text("OUTFIT KITA MANAGER", style="bold white")
        subtitle = Text("Kelola Produk Katalog", style="bold yellow")
        meta_text = Text(f"{meta} • {git}", style="dim white")
        console.print(
            Panel(
                Align.center(Text.assemble(title, "\n", subtitle, "\n", meta_text)),
                title="[bold black on yellow] DASHBOARD [/]",
        ...
        width = 72
        git_color = "red" if "Bukan Repo" in git else ("yellow" if "perubahan" in git.lower() else "green")
        print(box_line(width, color="yellow", top=True))
        print(box_center("OUTFIT KITA MANAGER", width, color="white", bold=True, border_color="yellow"))
        print(box_center("Kelola Produk Katalog", width, color="yellow", bold=True, border_color="yellow"))
        print(box_center(f"{meta} • {git}", width, color=git_color, bold=True, border_color="yellow"))
        print(box_line(width, color="yellow", bottom=True))


def show_menu():
    items = [
        ("1", "Tambah produk otomatis", "green", "+"),
        ("2", "Edit produk manual", "cyan", "edit"),
        ("3", "Lihat daftar produk", "yellow", "list"),
        ("4", "Hapus produk", "red", "del"),
        ("5", "Dashboard admin web", "magenta", "web"),
        ("6", "Preview katalog", "blue", "view"),
        ("7", "Sinkron GitHub", "green", "git"),
        ("8", "Setup Shopee API", "yellow", "api"),
        ("9", "Kelola kategori", "cyan", "cat"),
        ("0", "Keluar", "red", "exit"),
    ]

    if RICH:
        grid = Table.grid(padding=(0, 2))
        for no, label, _, _ in items:
            no_style = "bold red" if no == "0" else "bold cyan"
            grid.add_row(f"[{no_style}]{no}[/]", f"[bold white]{label}[/]")
        console.print(Panel(grid, title="[bold green]MENU[/]", border_style="green", box=box.ROUNDED, padding=(1, 2)))
    else:
        width = 72
        try:
            ...
            total_kategori = 0
        print()
        print(term_color("╔" + "═" * (width - 2) + "╗", "green", bold=True))
        print(term_color("║", "green", bold=True) + term_color(" MENU UTAMA ".center(width - 2), "green", bold=True) + term_color("║", "green", bold=True))
        print(term_color("╠" + "═" * (width - 2) + "╣", "green", bold=True))
        stats = f"Produk: {total_produk}  •  Kategori: {total_kategori}  •  Mode: Termux"
        print(term_color("║", "green", bold=True) + term_color(stats.center(width - 2), "yellow", bold=True) + term_color("║", "green", bold=True))
        print(term_color("╠" + "═" * (width - 2) + "╣", "green", bold=True))
        for no, label, color, icon in items:
            no_txt = term_color(f" {no} ", color, bold=True)
            icon_txt = term_color(f"{icon:<5}", color, bold=True)
            label_txt = term_color(label, "white", bold=True if no in {"5", "6"} else False)
            raw_len = 3 + 1 + 5 + 1 + len(label)
            padding = " " * max(0, width - 2 - raw_len)
            print(term_color("║", "green", bold=True) + no_txt + " " + icon_txt + " " + label_txt + padding + term_color("║", "green", bold=True))
        print(term_color("╚" + "═" * (width - 2) + "╝", "green", bold=True))
...
def input_category(auto_category, product_name):
    ...
    if RICH:
        ...
        for i, cat in enumerate(categories, start=1):
            label = f"{cat}  [dim](dipilih otomatis)[/]" if cat.lower() == auto_category.lower() else cat
...
def products_table(products):
    ...
    if RICH:
        ...
        table.add_column("No", justify="center", style="cyan", width=6, no_wrap=True)
        table.add_column("Nama Produk", style="white", min_width=22, ratio=3)
        table.add_column("Kategori", style="yellow", width=14)
        table.add_column("Harga", style="green", width=14)
        ...
        title = f" DAFTAR PRODUK ({len(rows)} ITEM) "
        print(term_color("║", "cyan", bold=True) + term_color(title.center(width - 2), "cyan", bold=True) + term_color("║", "cyan", bold=True))
        print(term_color("╠" + "═" * (width - 2) + "╣", "cyan", bold=True))
        header = f"{'NO':<7}{'KATEGORI':<15}{'HARGA':<15}{'PLATFORM':<11}NAMA PRODUK"
        print(term_color("║ ", "cyan", bold=True) + term_color(header.ljust(width - 4), "yellow", bold=True) + term_color(" ║", "cyan", bold=True))
        print(term_color("╟" + "─" * (width - 2) + "╢", "cyan", bold=True))
        for p in rows:
            ...
            row = (
                term_color(f"{no:<7}", "cyan", bold=True)
                + term_color(f"{kategori:<15}", "yellow")
                + term_color(f"{harga:<15}", "green", bold=True)
                + term_color(f"{platform:<11}", "magenta")
                + term_color(nama, "white")
            )
            plain_len = 7 + 15 + 15 + 11 + len(nama)
            print(term_color("║ ", "cyan", bold=True) + row + " " * max(0, width - 4 - plain_len) + term_color(" ║", "cyan", bold=True))
        print(term_color("╚" + "═" * (width - 2) + "╝", "cyan", bold=True))
...
def product_summary(p, title="Detail Produk"):
    ...
    if RICH:
        ...
        for label, value in rows:
            table.add_row(f"[bold cyan]{label}[/]", value)
        console.print(Panel(table, title=f"[bold yellow]{title}[/]", border_style="yellow", box=box.ROUNDED))
    else:
        width = 72
        print("\n" + box_line(width, color="yellow", top=True))
        print(box_center(title, width, color="yellow", bold=True, border_color="yellow"))
        print(term_color("├" + "─" * (width - 2) + "┤", "yellow", bold=True))
        rows = [
            ("No Produk", p["id"], "cyan"),
            ("Nama", short(p["name"], 50), "white"),
            ("Harga", p["price"], "green"),
            ("Kategori", p["category"], "yellow"),
            ("Platform", p["platform"], "magenta"),
            ("Gambar", short(p["image"], 48), "blue"),
            ("Link", short(p["link"], 48), "cyan"),
        ]
        for label, value, color in rows:
            label_text = f" {label:<10}: "
            plain_len = len(label_text) + len(str(value))
            print(term_color("│", "yellow", bold=True) + term_color(label_text, "cyan", bold=True) + term_color(str(value), color, bold=(label in {"Harga", "No Produk"})) + " " * max(0, width - 2 - plain_len) + term_color("│", "yellow", bold=True))
        print(box_line(width, color="yellow", bottom=True))
...
def show_product_before_category(pid, name, price, platform, link):
    ...
    if RICH:
        ...
        console.print(Panel(
            table,
            title="[bold yellow]Cek Data Produk[/]",
        ...
        width = 72
        print("\n" + box_line(width, color="magenta", top=True))
        print(box_center("CEK DATA PRODUK", width, color="magenta", bold=True, border_color="magenta"))
        print(term_color("├" + "─" * (width - 2) + "┤", "magenta", bold=True))
        rows = [
            ("No Produk", str(pid), "cyan"),
            ("Nama", short(name, 50), "white"),
            ("Harga", price or "Cek harga", "green"),
            ("Platform", platform or detect_platform(link), "yellow"),
            ("Link", short(link, 48), "blue"),
        ]
        for label, value, color in rows:
            label_text = f" {label:<10}: "
            plain_len = len(label_text) + len(str(value))
            print(term_color("│", "magenta", bold=True) + term_color(label_text, "cyan", bold=True) + term_color(str(value), color, bold=(label in {"No Produk", "Harga"})) + " " * max(0, width - 2 - plain_len) + term_color("│", "magenta", bold=True))
        print(box_line(width, color="magenta", bottom=True))
...
def build_bulk_product(products, pid, raw_line, link):
    ...
    if (not is_probably_product_name(name)) or is_placeholder_product_name(name, pid):
        warn(
            f"Nama produk no {pid} belum terbaca otomatis.\n"
            "Isi nama produk dulu supaya kategori otomatis bisa lebih tepat."
        )
        name = input_name(f"Produk {pid}")
    ...
    # Penting: nama produk ditampilkan sebelum pemilihan kategori, supaya tidak bingung
    # produk nomor ini sebenarnya barang apa.
...
def edit_product():
    ...
    while True:
        ...
        if RICH:
            ...
            edit_items = [("1", "Nama Produk", "cyan"), ("2", "Harga", "green"), ("3", "Kategori", "yellow"), ("4", "Link Gambar", "blue"), ("5", "Link Produk", "magenta"), ("0", "Simpan dan kembali", "red")]
            for no, label, color in edit_items:
                print(term_color(f"{no}. ", color, bold=True) + term_color(label, "white"))
...
def list_products():
    ...
    if RICH:
        ...
        print(term_color("\nCari berdasarkan nomor, nama, kategori, atau platform.", "cyan", bold=True))
    ...
    while True:
        ...
        if RICH:
            ...
            print("\n" + term_color(info_line, "yellow", bold=True))
        if total_pages == 1:
            break
...
def manage_categories():
    while True:
        ...
        if RICH:
            ...
            print("\n" + term_color("KATEGORI KATALOG", "cyan", bold=True))
            print(term_color("-" * 72, "cyan"))
            for i, cat in enumerate(categories, start=1):
                count = sum(1 for p in products if str(p.get("category", "")).lower() == cat.lower())
                print(term_color(f"{i:>2}. ", "cyan", bold=True) + term_color(f"{cat:<28}", "white") + term_color(f"{count} produk", "green", bold=True))
            print(term_color("-" * 72, "cyan"))
            menu_cat = [("1", "Tambah kategori", "green"), ("2", "Ubah nama kategori", "yellow"), ("3", "Hapus kategori", "red"), ("0", "Kembali", "cyan")]
            for no, label, color in menu_cat:
                print(term_color(f"{no}. ", color, bold=True) + term_color(label, "white"))
